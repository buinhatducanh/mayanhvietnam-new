...
const HERO_SLIDES = [
  {
    brand: 'Canon',
    product: 'EOS R6 Mark II Body',
    tagline: 'Canon EOS R6 Mark II body only — like-new 99%, sử dụng kỹ, còn bảo hành 6 tháng.',
    img: used[0].thumbnail,
  },
  {
    brand: 'Sony',
    product: 'Alpha A7 IV Body',
    tagline: 'Sony A7 IV body — like-new 95%, còn bảo hành chính hãng 4 tháng.',
    img: used[1].thumbnail,
  },
  {
    brand: 'Canon',
    product: 'RF 24-70mm f/2.8L',
    tagline: 'Lens Canon RF 24-70L IS USM — like-new 98%, đã qua sử dụng kỹ lưỡng.',
    img: used[2].thumbnail,
  },
  {
    brand: 'DJI',
    product: 'Mavic 3 Pro',
    tagline: 'Mavic 3 Pro Fly More Combo (DJI RC) — like-new 96%, bay dưới 8h.',
    img: used[3].thumbnail,
  }
]

const BRANDS = ['Sony Cũ', 'Canon Cũ', 'Fujifilm Cũ', 'Nikon Cũ', 'Ống kính Cũ']

const FILTERS = [
  { group: 'Mức giá', options: ['Dưới 5 triệu', '5–15 triệu', '15–30 triệu', 'Trên 30 triệu'] }
]

export function UsedProductsPage({ 
  onNavigate,
  onAddToCart,
  cartCount = 0
}: { 
  ...
  useSEO(
    'Sản phẩm Cũ (Like New) Chính hãng | Máy Ảnh Việt Nam',
    'Mua bán máy ảnh, ống kính cũ (Like New). Cam kết ngoại hình đẹp, zin 100%, bảo hành dài hạn từ Máy Ảnh Việt Nam.'
  )
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <RefreshCw size={1000} strokeWidth={0.5} color="#141414" />
      </div>

      <SharedNavbar activeItem="Sản phẩm cũ" onNavigate={onNavigate} cartCount={cartCount} />
      ...
      <main style={{ paddingTop: '72px', position: 'relative', zIndex: 10 }}>
               ...
               <div style={{ 
                 ...
                 justifyContent: 'center', zIndex: 10, background: 'linear-gradient(to right, #F6F6F2 80%, transparent)'
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px' }}>
                    <ShieldCheck size={16} color="#E86A24" />
                    <span style={{ fontSize: '14px', letterSpacing: '0.2em', textTransform: 'uppercase', color: '#E86A24', fontWeight: 600 }}>{slide.brand}</span>
                  </div>
        ...
        <section style={{ padding: '80px 4vw 40px 4vw' }}>
          <h3 style={{ textAlign: 'center', fontSize: '24px', fontWeight: 400, fontFamily: 'var(--font-display)', marginBottom: '48px', color: '#3A3A3A' }}>
            Khám phá Thiết bị Cũ - Được Kiểm Định
          </h3>
