...
import { Star, ShoppingCart, Aperture, ChevronLeft, ChevronRight, LocateFixed } from 'lucide-react'
import { actionCameras } from '../../../../lib/mock-data'

// Data
const HERO_SLIDES = [
  {
    brand: 'GoPro',
    product: 'Hero 13 Black',
    tagline: 'Chinh phục mọi giới hạn. Ghi lại từng khoảnh khắc phiêu lưu với 5.3K 60fps.',
    img: actionCameras[0].thumbnail,
  },
  {
    brand: 'DJI',
    product: 'Osmo Action 4 Adventure Combo',
    tagline: 'Cảm biến 1/1.3" lớn nhất phân khúc. Chống nước 18m, pin 160 phút.',
    img: actionCameras[1].thumbnail,
  },
  {
    brand: 'DJI',
    product: 'Osmo Pocket 4 Creator Combo',
    tagline: 'Camera gimbal bỏ túi hoàn hảo cho những nhà sáng tạo nội dung.',
    img: actionCameras[2].thumbnail,
  },
  {
    brand: 'DJI',
    product: 'Osmo Nano Standard Combo',
    tagline: 'Action camera nhỏ gọn nhất. Chỉ 36g, 4K 120fps, chống nước 10m.',
    img: actionCameras[3].thumbnail,
  }
]

const BRANDS = ['DJI', 'GoPro', 'Insta360', 'Sony']

const FILTERS = [
  { group: 'Mức giá', options: ['Dưới 5 triệu', '5–10 triệu', '10–15 triệu', 'Trên 15 triệu'] }
]

export function ActionCameraPage({ 
  onNavigate,
  onAddToCart,
  cartCount = 0
}: { 
  ...
  useSEO(
    'Action Camera & Camera 360 | Máy Ảnh Việt Nam',
    'Camera hành động GoPro, Insta360, DJI Osmo Action chính hãng. Chống rung đỉnh cao, ghi hình sắc nét cho mọi chuyến phiêu lưu.'
  )
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <LocateFixed size={1000} strokeWidth={0.5} color="#141414" />
      </div>

      <SharedNavbar activeItem="Action Camera" onNavigate={onNavigate} cartCount={cartCount} />
      ...
      <main style={{ paddingTop: '72px', position: 'relative', zIndex: 10 }}>
        ...
        <section style={{ padding: '80px 4vw 40px 4vw' }}>
          <h3 style={{ textAlign: 'center', fontSize: '24px', fontWeight: 400, fontFamily: 'var(--font-display)', marginBottom: '48px', color: '#3A3A3A' }}>
            Thiết bị hành động chuyên nghiệp
          </h3>
                 ...
                 <div style={{ transition: 'transform 0.5s cubic-bezier(0.16, 1, 0.3, 1)' }} className="group-hover:-translate-y-2">
                   ...
                   <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '24px' }}>
                     <div style={{ display: 'flex', color: '#E86A24' }}>
                       {[...Array(5)].map((_, i) => (
                         <Star key={i} size={14} fill={i < Math.floor(p.rating?.average ?? 0) ? 'currentColor' : 'none'} strokeWidth={i < Math.floor(p.rating?.average ?? 0) ? 0 : 2} />
                       ))}
                     </div>
                   ...
                   <div style={{ display: 'flex', gap: '12px' }}>
                      ...
                      <button onClick={(e) => { e.stopPropagation(); onAddToCart?.(); }} style={{ padding: '16px 24px', background: '#E86A24', color: '#FFF', border: 'none', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.3s ease' }} className="hover:bg-[#d45d1d] hover:shadow-[0_4px_16px_rgba(232,106,36,0.3)]">
